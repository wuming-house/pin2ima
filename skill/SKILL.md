...
从知识库中拉取近期收藏，按话题聚类，生成选题洞察。触发词："选题报告""选题速览""本周选题""看看有什么选题"。
...
基于提取到的标题、标签、摘要，AI 自动完成：

1. **话题聚类**：标题/标签/摘要中出现相同/相近关键词的归为一组
2. **热度排序**：同一话题下文章数越多越靠前
3. **选题建议**：高频话题 → 建议跟进；独有角度 → 差异化切入
4. **生成推荐标题**：基于聚类结果和素材内容，为每个高频话题生成 2-3 个可直接使用的文章标题
