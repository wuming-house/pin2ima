...
2. **User says "知识库" without naming** → default to **"公众号文章知识库"**
...
RESPONSE=$(curl -s "https://ima.qq.com/openapi/wiki/v1/search_knowledge_base" \
  ...
  -d '{"query":"\u516c\u4f17\u53f7\u6587\u7ae0\u77e5\u8bc6\u5e93","cursor":"","limit":5}')
```
...
根据 URL 的域名自动判断保存到哪个知识库，适合用不同知识库管理不同选题方向。

**默认路由规则：**

| URL 特征 | 目标知识库 |
|----------|-----------|
| `mp.weixin.qq.com` | 公众号文章知识库 |
| `zhuanlan.zhihu.com` / `zhihu.com` | 知乎选题知识库 |
| `github.com` | GitHub 项目知识库 |
| `36kr.com` / `mp.weixin.qq.com/s/*finance*` | 商业财经知识库 |
| 其他 | 默认知识库（可配置） |

**实现方式：** 从 URL 中提取 hostname，匹配路由表，调用 `search_knowledge_base` 查找对应知识库的 ID。路由表可写死在 Skill 中，用户也可在对话中覆盖。
...
- **视频内容无法直接存入 IMA 知识库**，`import_urls` 仅保存一个 URL 书签
- 本工作流的目的是：将视频的核心信息提取成摘要，**通过笔记永久保存到知识库中**
- 笔记 + URL 双保险：笔记存摘要（可搜索可问答），URL 存入口（可直接点开看原片）
...
# Step D: 保存 URL 入口（可选，笔记里已有链接）
# URL 本身也可以存，方便直接点开
curl -s "https://ima.qq.com/openapi/wiki/v1/import_urls" \
  -H "ima-openapi-clientid: $CLIENT_ID" \
  -H "ima-openapi-apikey: $API_KEY" \
  -H "ima-openapi-ctx: skill_version=1.0.0" \
  -H "Content-Type: application/json" \
  -d "{\"knowledge_base_id\":\"$KB_ID\",\"urls\":[\"$VIDEO_URL\"]}"
```
...
|---------|---------|----------|
| Target knowledge base name | "公众号文章知识库" | User specifies in conversation |
