...
从知识库 + 全网热门内容联合生成选题洞察。触发词："选题报告""选题速览""本周选题""看看有什么选题"。

#### Step 0: 询问联网搜索偏好（必须执行）

当用户说选题速览后，**先不要开始搜索**，而是向用户确认两个信息：
...
两部分结合后，输出更全面的选题洞察。在开始搜索全网热门内容之前，想确认一下：

① 需要搜索哪类内容的热门趋势？
   比如：AI 工具 / 开源项目 / 编程教程 / 自媒体运营 / 全部热门
...
**交互规则：**
- 两个问题合在一起问，用户一次回答，不要分两次
- 问题用简洁的选项式提问，降低用户思考成本
- 如果用户未指定（如只说"选题速览"），以下方默认值执行，不追问：
  - 内容类型默认：**全部**
  - 时间范围默认：**本周**
...
# 拉取最近收藏（按更新时间降序，取前 50 条）
LIST=$(curl -s "https://ima.qq.com/openapi/wiki/v1/get_knowledge_list" \
  -H "ima-openapi-clientid: $CLIENT_ID" \
  -H "ima-openapi-apikey: $API_KEY" \
  -H "ima-openapi-ctx: skill_version=1.0.0" \
  -H "Content-Type: application/json" \
...
**原文链接获取：** 读取 `~/.config/ima/url_index.json`，用 `media_id` 匹配原文 URL。

**B2. 联网搜索全网热门内容：**

基于 Step 0 用户指定的**内容类型**和**时间范围**，用 WebSearch 搜索全网热门内容：

```
搜索关键词构造规则：
  格式: "{内容类型} 热门 {时间范围}"
  示例: "AI 工具 热门 本周"、"开源项目 热点 最近 3 天"
```
...
1. **话题聚类**：知识库标题 + 全网热点标题，按关键词归为一组
2. **双区标注**：同一话题下区分「📦 知识库已有」（你的收藏）和「🔥 全网热议」（外部热点）
3. **热度排序**：话题下内容越多越靠前
4. **生成推荐标题**：结合知识库素材 + 全网热点，生成 2-3 个可直接使用的标题

#### Step C-2: 关键词（词云图，必须包含在「高频话题」区块顶部）

在「高频话题」区块的最顶部，渲染一张**关键词**词云图表，直观呈现本期素材的主题分布。

**数据来源：** 知识库全部标题 + 全网热门标题，合并后做关键词频次统计。

**关键词抽取（零依赖，无需 jieba / wordcloud 等第三方包）：**
1. 中文分词用 **n-gram 共现法**：对每个标题生成长度 2/3/4 的连续子串，统计**文档频次（df = 包含该子串的标题数）**，仅保留 `df ≥ 2` 的子串
2. 英文 / 字母数字混合词用正则 `[A-Za-z][A-Za-z0-9+.]{1,}` 提取（如 `ClaudeCode`、`Codex`、`GitHub`、`Agent`）
3. 数字 + 单位组合用正则 `\d+(?:\.\d+)?\s*[万千Kk]\s*(?:Star|星)?` 提取（如 `11.9万Star`、`522 K`）
4. **冗余去除**：若较短词是较长词的子串且较长词 df 不低于它，则丢弃较短词（如 `开源项` ⊂ `开源项目`）
5. **停用词过滤**：剔除 `的/了/这个/如何/发布/推出` 等无意义 2 字组合，以及含数字的年份碎片（`2026` 等）
6. 取频次 Top 40~46 作为词云词条，按频次映射到字号（最小 ~14px，最大 ~64px）

**渲染方式（HTML 报告内联，离线自包含）：**
- 内联 **wordcloud2.js**（MIT，约 11KB，生成报告时从 CDN 拉取一次后内联进 HTML，无需运行时联网）
- 画布背景透明，配色使用橙金渐变：高频词 `#f5c451`（金）、中频 `#ffb05c`、低频 `#ff8c3b`
- `rotateRatio: 0.32` 轻微倾斜，`shrinkToFit: true` 自适应铺满
- **兜底**：若 wordcloud2 未加载成功，降级为 CSS 标签云（同样按频次着色、字号），保证任何环境都能看到词云

**文案：** 词云卡片标题「☁️ 关键词」，副标题说明「聚合自知识库 N 条标题 + 全网今日热门标题，字号越大代表出现频次越高」。

#### 输出模板

```
📊 选题速览（{日期范围}）
知识库：选题知识库 | 近7天共 {N} 条收藏

🔥 高频话题（≥2篇）
{序号}. {话题}（知识库 {N}篇 + 全网 {M}条）← 建议跟进
   📦 知识库已有
   ├─ {标题1}
   └─ {标题2}
   🔥 全网热议
...
**推荐标题生成规则：**
- 每个高频话题生成 2-3 个标题
- **标题生成需同时参考知识库素材 + 全网热点**，确保既接地气又有时效性
- 如果全网热点中有知识库未覆盖的新角度，优先出跨界融合型标题
- 每个标题下方用 `↩ 参考：` 列出引用文章（含原文链接或 IMA 内链）
- **链接规则：**
  - 📦 知识库文章 → 有 url_index.json 匹配用真实链接，无匹配用 `<span onclick>` 点击复制标题
  - 🔥 全网搜索结果 → **必须使用 WebSearch 返回的真实 URL**，用 `<a href="URL" target="_blank">` 可点击直接跳转
  - 写作思路要具体落地：**开头怎么写 / 用什么结构 / 亮点在哪里**
- 标题需包含**数字**或**对比**

#### 触发方式

1. **对话触发**：用户说"选题报告/选题速览/本周选题/看看有什么选题"，Agent 执行上述 Step A-C
2. **定时任务**：结合 WorkBuddy automation，每天定时生成报告
